...
fn parse_tagged_template() {
    let mut parser = create_parser("#[abstract] template User {}");
    match parser.parse() {
        Ok(program) => {
...
fn parse_tagged_template_field() {
    let mut parser = create_parser("template User { #[primary_key] #[unique] id = string; }");
    match parser.parse() {
        Ok(program) => {
...
fn parse_empty_template() {
    let mut parser = create_parser("template User {}");
    match parser.parse() {
        Ok(program) => {
            assert_eq!(program.0.len(), 1);
            let Statement::Template {
...
fn parse_single_field_template() {
    let mut parser = create_parser("template User { name = string; }");
    match parser.parse() {
        Ok(program) => {
            let Statement::Template {
                name,
...
fn parse_missing_paren_template() {
    let mut parser = create_parser("template Invalid { name = string;");
    expect_missing_paren(&mut parser);
}

#[test]
fn parse_anonymus_generate() {
    let mut parser = create_parser("@generate _ [10] { name = string; price = float; }");
    match parser.parse() {
        Ok(program) => {
...
#[test]
fn parse_generate() {
    let mut parser = create_parser("@generate User [10];");
    match parser.parse() {
        Ok(program) => {
            let Statement::Generate {
                template_name,
...
#[test]
fn parse_empty_enum() {
    let mut parser = create_parser("enum Role {}");
    match parser.parse() {
        Ok(program) => {
            let Statement::Enum {
                name,
...
fn parse_single_variant_enum() {
    let mut parser = create_parser("enum Role { User; }");
    match parser.parse() {
        Ok(program) => {
            let Statement::Enum {
                name,
...
#[test]
fn parse_list_type() {
    let mut parser = create_parser("[int][range=1..=5];");
    match parser.parse() {
        Ok(program) => {
...
fn parse_list_expression() {
    let mut parser = create_parser("[1 => 5; \"John\"; true => 25];");
    match parser.parse() {
        Ok(program) => {
            let Statement::Expression(expr_stmt) = &program.0[0] else {
                panic!("Expected Expression statement");
...
#[test]
fn parse_enum() {
    let mut parser = create_parser("enum Role { User; Admin; Moderator; }");
    match parser.parse() {
        Ok(program) => {
            let Statement::Enum {
                name,
...
fn parse_infix_expression() {
    let mut parser = create_parser("2 + 10 * 20;");
    match parser.parse() {
        Ok(program) => {
            let Statement::Expression(expr_stmt) = &program.0[0] else {
                panic!("Expected Expression statement");
...
fn parse_grouped_expression() {
    let mut parser = create_parser("(2 + 3) * 5;");
    match parser.parse() {
        Ok(program) => {
            let Statement::Expression(expr_stmt) = &program.0[0] else {
                panic!("Expected Expression statement");
...
fn parse_prefix_expression() {
    let mut parser = create_parser("-5; !true;");
    match parser.parse() {
        Ok(program) => {
            assert_eq!(program.0.len(), 2);
...
#[test]
fn parse_directive() {
    let mut parser = create_parser("@output csv;");
    match parser.parse() {
        Ok(program) => {
...
fn parse_output_path() {
    let mut parser = create_parser("@output_path \"./example.csv\";");
    match parser.parse() {
        Ok(program) => {
...
fn parse_pattern_dollar_case() {
    let mut parser = create_parser("string_pattern \"dollar$$$$$$$$$$$$$$$$${aa}\";");
    match parser.parse() {
        Ok(program) => {
            let Statement::Expression(expr_stmt) = &program.0[0] else {
                panic!("Expected Expression statement");
...
fn parse_string_pattern() {
    let mut parser = create_parser("string_pattern \"testa$}${aaa[10]}john${A[25]##[13]}\";");
    match parser.parse() {
        Ok(program) => {
...
fn parse_directive_options() {
    let mut parser = create_parser("@output csv { delimiter = \";\"; }");
    match parser.parse() {
        Ok(program) => {
            let Statement::OutputDirective {
                argument, options, ..
