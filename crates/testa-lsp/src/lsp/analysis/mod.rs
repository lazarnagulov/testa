...
use testa_core::{analyser::{SemanticAnalyser, symbol_table::SymbolTable}, ast::{Program, Statement}, lexer::Lexer, parser::Parser};
...
impl AnalysisEngine {
    ...
    fn resolve_imports(ast: &Program, file_path: Option<&Path>) -> ResolvedUnit {
        if let Some(path) = file_path {
            let import_names: Vec<String> = ast.0.iter().filter_map(|stmt| {
