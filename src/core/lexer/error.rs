...
use crate::core::{diagnostics::{Diagnostic, DiagnosticCode::{self}}, utils::span::Span};
...
impl fmt::Display for LexerError {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::MissingChar { expected, .. } => write!(f, "Expected '{}' but reached end of input", expected)
        }
    }
}
    ...
    pub fn to_diagnostic(&self) -> Diagnostic {
        match self {
            Self::InvalidBuiltIn { span, value } => {
                Diagnostic::error(span.clone(), format!("Invalid built-in type '{}'", value))
            ...
            Self::InvalidToken { span, token } => {
                Diagnostic::error(span.clone(), format!("Unexpected character '{}'", token))
                    .with_code(DiagnosticCode::UnexpectedCharacter)
            }
            
            Self::InvalidDirective { span, value } => {
                Diagnostic::error(span.clone(), format!("Invalid directive '@{}'", value))
            ...
            Self::InvalidNumberLiteral { span, value } => {
                Diagnostic::error(span.clone(), format!("Invalid number literal '{}'", value))
                    .with_code(DiagnosticCode::InvalidNumberLiteral)
                    .with_hint("Numbers must be valid integers or floats (e.g., 42, 3.14)")
            }
            
            Self::MissingChar { span, expected } => {
                Diagnostic::error(span.clone(), format!("Expected '{}' but reached end of input", expected))
